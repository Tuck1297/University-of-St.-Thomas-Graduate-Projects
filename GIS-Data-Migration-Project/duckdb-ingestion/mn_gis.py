...
def run(conn, existed):
    ...
    forest_dir = os.path.join("downloads", "shp_struc_state_forest_campgrounds")

    if not os.path.exists(gis_dir):
        print(f"Error: MN GIS Units directory not found at {gis_dir}")
        return
    ...
    if existed:
        try:
            ...
            if schema_exists:
                # Check for the units table
                if not handle_existing_data(conn, f"{schema_name}.dnr_management_units_prk", "Minnesota GIS Units"):
                    return
        except Exception:
            # If table doesn't exist, proceed
            pass

    print("Running Minnesota GIS Units job")
    ...
    # List of shapefiles to import from bdry directory (minus boundaries)
    shapefiles = [
    ...
    # Import forest campgrounds if the directory exists
    if os.path.exists(forest_dir):
        ...
        for shp_file in forest_shapefiles:
            table_name = shp_file.replace(".shp", "")
            file_path = os.path.join(forest_dir, shp_file)
            ...
            if os.path.exists(file_path):
                ...
                print(f"Warning: File {shp_file} not found in {forest_dir}")

    print("Minnesota GIS Units job completed successfully.")
